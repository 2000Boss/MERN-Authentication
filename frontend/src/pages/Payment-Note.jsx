import React, { useState, useEffect } from "react";
import Cookies from "js-cookie";
import {
  CardNumberElement,
  CardExpiryElement,
  CardCvcElement,
  useStripe,
  useElements,
} from "@stripe/react-stripe-js";
// import { fetchFromAPI } from "../../../helpers";
import Request from "../services/Axios-CRUD";

import { useLocation, useNavigate } from "react-router-dom";

const withLocation = (Component) => (props) => {
  const location = useLocation();

  return <Component {...props} location={location} />;
};

const CustomCheckout = ({ subscriptionType }) => {
  const userLoggedIn = Cookies.get("logged_in");
  const [getLoad, setLoad] = useState(false);
  const [error, setError] = useState(null);
  const [clientSecret, setClienSecret] = useState(null);
  const [cards, setCards] = useState(null);
  const [payment, setPaymentCard] = useState("");
  const [saveCard, setSavedCard] = useState(false);
  const [paymentIntentId, setPaymentIntentId] = useState(null);
  const stripe = useStripe();
  const elements = useElements();
  const navigate = useNavigate();

  useEffect(() => {
    if (userLoggedIn) {
      const savedCards = async () => {
        try {
          const cardsList = await Request.paymentMethods();
          setCards(cardsList);
        } catch (error) {
          console.log(error);
        }
      };
      savedCards();
    }

    const body = {
      subscriptionType: "Basic",
      email: "madiev.abboss@gmail.com",
    };

    const customCheckout = async () => {
      const { clientSecret, id } = await Request.createPayment(body);

      setClienSecret(clientSecret);
      setPaymentIntentId(id);
    };

    customCheckout();
  }, [subscriptionType, userLoggedIn]);

  const handleCheckout = async () => {
    setLoad(true);
    let si;
    // check if userLoggedIn has selected to save card
    if (saveCard) {
      // make to create a setup intent
      si = await Request.savePayment();
    }
    const payload = await stripe.confirmCardPayment(clientSecret, {
      payment_method: {
        card: elements.getElement(CardNumberElement),
      },
    });

    if (payload.error) {
      setError(`Payment Failed: ${payload.error.message}`);
    } else {
      if (saveCard && si) {
        // send the customers card details to be saved with stripe
        await stripe.confirmCardSetup(si.client_secret, {
          payment_method: {
            card: elements.getElement(CardNumberElement),
          },
        });
        navigate("/success");
      } else {
        navigate("/success");
      }
    }
  };

  const savedCardCheckout = async () => {
    setLoad(true);
    // update the payment intent to incude the customer parameter
    const { clientSecret } = await Request.updatePayment(paymentIntentId);

    const payload = await stripe.confirmCardPayment(clientSecret, {
      payment_method: payment,
    });

    if (payload.error) {
      setError(`Payment Failed: ${payload.error.message}`);
      setLoad(false);
    } else {
      navigate("/success");
    }
  };

  const cardHandleChange = (event) => {
    const { error } = event;
    setError(error ? error.message : "");
  };

  const cardStyle = {
    style: {
      base: {
        color: "#000",
        fontFamily: "Roboto, sans-serif",
        fontSmoothing: "antialiased",
        fontSize: "16px",
        "::placeholder": {
          color: "#606060",
        },
      },
      invalid: {
        color: "#fa755a",
        iconColor: "#fa755a",
      },
    },
  };

  let cardOption;

  if (cards) {
    cardOption = cards.map((card) => {
      const {
        card: { brand, last4, exp_month, exp_year },
      } = card;
      return (
        <option key={card.id} value={card.id}>
          {`${brand}/ **** **** **** ${last4} ${exp_month}/${exp_year}`}
        </option>
      );
    });
    cardOption.unshift(
      <option key="Select a card" value="">
        Select A Card
      </option>
    );
  }

  return (
    <div>
      {userLoggedIn && cards && cards.length > 0 && (
        <div>
          <h4>Pay with saved card</h4>
          <select
            value={payment}
            onChange={(e) => setPaymentCard(e.target.value)}
          >
            {cardOption}
          </select>
          <button
            type="submit"
            disabled={getLoad || !payment}
            className="button is-black nomad-btn submit saved-card-btn"
            onClick={() => savedCardCheckout()}
          >
            {getLoad ? "getLoad" : "PAY WITH SAVED CARD"}
          </button>
        </div>
      )}
      <h4>Enter Payment Details</h4>
      <div className="stripe-card">
        <CardNumberElement
          className="card-element"
          options={cardStyle}
          onChange={cardHandleChange}
        />
      </div>
      <div className="stripe-card">
        <CardExpiryElement
          className="card-element"
          options={cardStyle}
          onChange={cardHandleChange}
        />
      </div>
      <div className="stripe-card">
        <CardCvcElement
          className="card-element"
          options={cardStyle}
          onChange={cardHandleChange}
        />
      </div>
      {userLoggedIn && (
        <div className="save-card">
          <label>Save Card</label>
          <input
            type="checkbox"
            checked={saveCard}
            onChange={(e) => setSavedCard(e.target.checked)}
          />
        </div>
      )}
      <div className="submit-btn">
        <button
          disabled={getLoad}
          className="button is-black nomad-btn submit"
          onClick={() => handleCheckout()}
        >
          {getLoad ? "getLoad" : "PAY"}
        </button>
      </div>
      {error && <p className="error-message">{error}</p>}
    </div>
  );
};

export default withLocation(CustomCheckout);
